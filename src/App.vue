<template>
  ...
  >
    ...
    >
      <div
        class="max-w-3xl mx-auto px-5 h-16 flex items-center justify-between"
      >
        <div class="flex items-center gap-2 cursor-pointer" @click="goHome">
          <div
            class="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-white"
          >
    ...
    <main class="flex-grow max-w-3xl w-full mx-auto px-5 py-8">
      <div
        class="mb-6 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"
      >
        <h1 class="text-lg font-bold text-slate-900 mb-1">
          환영합니다! 자유로운 소통 공간입니다.
        </h1>
        <p class="text-slate-500 text-xs sm:text-sm leading-relaxed">
          회원가입이나 로그인 없이 비밀번호만으로 간편하고 안전하게 글을
          작성하고 조회할 수 있습니다.
        </p>
      </div>
      ...
      >
        ...
        <form
          @submit.prevent="view === 'create' ? submitCreate() : submitEdit()"
          class="space-y-5"
        >
          <div>
            <label
